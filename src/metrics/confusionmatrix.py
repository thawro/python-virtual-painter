...
def lazy_property(fn: Callable) -> Callable:
    ...
    def _lazy_property(self) -> Any:
        ...
        elif hasattr(self, "_recalculate") and fn.__name__ not in self._recalculate.keys():
...
class ConfusionMatrix:
    ...
    def add(self, y_pred: np.ndarray, y_true: np.ndarray) -> None:
        ...
        assert y_pred.shape[0] == y_true.shape[0], "num of y_true and y_pred outputs do not match"
        ...
        assert max(y_pred) < self.num_classes and min(y_pred) >= 0, "y_pred are not in [0, k-1]"
        assert max(y_true) < self.num_classes and min(y_true) >= 0, "y_true are not in [0, k-1]"
